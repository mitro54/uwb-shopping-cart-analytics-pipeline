...
===============================================
Streamlit dashboard for UWB positioning accuracy analysis.

Summary stats come from the f_paikannustarkkuus gold dbt model (parquet).
Raw ping data is queried on demand from silver_device_diagnostics (parquet)
via an in-memory DuckDB connection — same pattern as overnight_explorer.py.
...
def fetch_device_night(node_id: str, yo_paiva: str, cx: float, cy: float) -> pl.DataFrame:
    """Raw pings for one device on one night, with dx/dy/dist computed."""
    conn = _silver_conn()
    ...
    arrow = conn.execute(f"""
        ...
        FROM tz
        WHERE
            (CAST(aika_hki AS DATE) = CAST('{yo_paiva}' AS DATE)
             AND EXTRACT('hour' FROM aika_hki) >= 22)
            OR
            (CAST(aika_hki AS DATE) = CAST('{next_date}' AS DATE)
...
# ---------------------------------------------------------------------------
# All-devices table + RMSE bar chart
# ---------------------------------------------------------------------------
...
with left:
    ...
    # Negate dy so scatter and floor plan share the same visual orientation
    scatter_fig = go.Figure()
    scatter_fig.add_trace(go.Scattergl(
        x=dx_arr, y=[-d for d in dy_arr],
        mode="markers",
...
with right:
    ...
    for radius, color, label in [
        ...
        err_fig.add_vline(
            ...
            annotation_text=label, annotation_position="top right", annotation_font_size=11,
        )
...
# ── Floor plan ────────────────────────────────────────────────────────────────
st.markdown('<div class="section-title">🗺️ Laitteen sijainti pohjapiirustuksessa</div>', unsafe_allow_html=True)
if IMAGE_PATH.exists():
    img = load_image(IMAGE_PATH)
    fp_fig = go.Figure()
    fp_fig.add_layout_image(dict(
        source=img, xref="x", yref="y",
        x=0, y=0, sizex=MAP_MAX_X, sizey=MAP_MAX_Y,
    ...
    fp_fig.add_trace(go.Scattergl(
        x=df_raw["x"].to_list(), y=df_raw["y"].to_list(),
        mode="markers",
        marker=dict(size=3, color=tunti_arr, colorscale="Viridis", opacity=0.45,
                    colorbar=dict(title="Tunti", thickness=10, len=0.6), line=dict(width=0)),
        name="Havainnot",
    ...
    fp_fig.update_layout(
        ...
        legend=dict(orientation="h", yanchor="top", y=-0.02, xanchor="left", x=0, font=dict(size=11)),
    )
...
drift_fig.add_trace(go.Scattergl(
    x=aika_arr, y=dx_arr,
    mode="markers", marker=dict(size=2, color="#6366f1", opacity=0.3), name="ΔX",
))
drift_fig.add_trace(go.Scattergl(
    x=aika_arr, y=[-d for d in dy_arr],
    mode="markers", marker=dict(size=2, color="#ec4899", opacity=0.3), name="ΔY",
))
drift_fig.add_trace(go.Scatter(
    x=aika_arr, y=trend_x, mode="lines", line=dict(color="#6366f1", width=2),
    name=f"Trendi X ({r['drift_x_cmh']:+.1f} cm/h)",
))
drift_fig.add_trace(go.Scatter(
    x=aika_arr, y=[-v for v in trend_y], mode="lines", line=dict(color="#ec4899", width=2),
...
if len(df_node) > 1:
    ...
    for col_name, color, label in [
        ...
        cep_fig.add_trace(go.Scatter(
            ...
            mode="lines+markers", line=dict(color=color, width=2), marker=dict(size=6), name=label,
        ))
    cep_fig.update_layout(
        xaxis=dict(title="Yöpäivä"), yaxis=dict(title="cm"),
    ...
    mnight_drift = go.Figure()
    mnight_drift.add_trace(go.Scatter(
        x=dates, y=df_node["drift_x_cmh"].to_list(),
        mode="lines+markers", line=dict(color="#6366f1", width=2), marker=dict(size=6), name="Drift X",
    ))
    mnight_drift.add_trace(go.Scatter(
        x=dates, y=df_node["drift_y_cmh"].to_list(),
        mode="lines+markers", line=dict(color="#ec4899", width=2), marker=dict(size=6), name="Drift Y",
    ))
    mnight_drift.add_hline(y=0, line_color="#94a3b8", line_width=1)
    mnight_drift.update_layout(
    ...
    st.plotly_chart(mnight_drift, use_container_width=True)
